...
<script>
  ...
  function loadItemsProducts() {
    ...
    var url = '<%=request.getContextPath()%>/services/public/products/page/' + START_COUNT_PRODUCTS
        + '/' + MAX_PRODUCTS
        + '/<c:out value="${requestScope.MERCHANT_STORE.code}"/>/<c:out value="${requestScope.LANGUAGE.code}"/>/manufacturer/<c:out value="${manufacturer.id}"/>';
...
<div id="mainContent" class="container">
            ...
            <div class="fixed-image section dark-translucent-bg parallax-bg-3">
                <div class="container">
                    <h2 class="shop-banner-title lead"><c:out
                            value="${manufacturer.description.name}"/></h2>
                </div>
            </div>
        </c:if>
    ...
    <c:if test="${manufacturer.description.description!=null}">
        <div class="row">
            <p class="lead"><c:out value="${manufacturer.description.description}"
                                   escapeXml="false"/></p>
        </div>
    </c:if>
