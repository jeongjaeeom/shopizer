    ...
    <form class="panel-body">
        ...
        <div class="row"
            ...
            <div class="form-group col-sm-12">
                ...
                <div class="form-control" id="creditcard_card_number"
                     name="payment['creditcard_card_number']" autocomplete="off"></div>
...
<script type="text/javascript">
        ...
        hostedFieldsInstance.on('validityChange', function (event) {
          ...
          if (field.isValid) {
            ...
            if (event.emittedBy === 'expirationMonth') {
              ...
              $('#expiration-month').next('span').text(
                  getOrderValidationMessage('invalid_expiry_month'));

            }
            if (event.emittedBy === 'expirationYear') {
              $form.find('button').prop('disabled', true);
              $('#expiration-year').next('span').text(
                  getOrderValidationMessage('invalid_expiry_year'));
            }
            // Add helper text for an invalid card number
            if (event.emittedBy === 'number') {
              $form.find('button').prop('disabled', true);
              $('#creditcard_card_number').next('span').text(
                  getOrderValidationMessage('invalid_number'));
            }
          }
  ...
  function initBraintreePayment(div, data, defaultValue) {
    ...
    hfInstance.tokenize(function (err, payload) {
      ...
      var token = payload.nonce;
      // Insert the token into the form so it gets submitted to the server
      var tokenField = '<input type="hidden" name="payment[\'paymentToken\']" value="' + token
          + '" /><input type="hidden" name="payment[\'null_creditcard\']" value="null_creditcard"/>';
      $form.append(tokenField);
      log(tokenField);
      //alert('PaymentToken ' + token);
      // and submit
...
<script>$.ajax({
  ...
  success: function (data) {
    populateData($('#expiration-year-select'), data,
        '${order.payment['creditcard_card_expirationyear']}');
  }
})</script>
<script>$.ajax({
  ...
  type: "GET",
  success: function (data) {
    populateData($('#expiration-month-select'), data,
        '${order.payment['creditcard_card_expirationmonth']}');
  }
})</script>
