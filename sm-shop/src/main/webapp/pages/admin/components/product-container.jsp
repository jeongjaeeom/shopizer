...
<script>
  ...
  isc.RestDataSource.create({
    ...
    operationBindings: [
      ...
      },
      {
        operationType: "fetch",
        dataProtocol: "postParams",
        dataURL: "<c:url value="${containerFetchUrl}" />"
      },
      {
        operationType: "add",
        dataProtocol: "postParams",
        dataURL: "<c:url value="${containerAddUrl}" />"
      },
      {
        operationType: "update",
        dataProtocol: "postParams",
        dataURL: "<c:url value="${containerUpdateUrl}" />"
      }
    ],
  ...
  isc.TreeGrid.create({
    ...
    data: isc.Tree.create({
      ...
      data: [
        ...
        {
          categoryId: '<c:out value="${category.category.id}" />',
          parentId: '<c:choose><c:when test="${category.category.parent!=null}"><c:out value="${category.category.parent.id}" /></c:when><c:otherwise>-1</c:otherwise></c:choose>',
          Name: '<c:out value="${category.descriptions[0].name}" />',
          isFolder: true
        }
    ...
    createRecordComponent: function (record, colNum) {
      ...
      if (fieldName == "buttonField") {
        var button = isc.IButton.create({
          ...
          click: function () {
            var url = '<c:url value="/admin/products/editProduct.html" />?id='
                + record["productId"];
  ...
  isc.ListGrid.create({
    ...
    removeData: function () {
      if (confirm(
          '<s:message code="label.entity.remove.confirm" text="Do you really want to remove this record ?" />')) {
