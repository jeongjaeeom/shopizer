...
function getZones(countryCode, zoneCode) {
  ...
  $.ajax({
    ...
    success: function (responseObj) {
      ...
      if ((responseObj.response.status == 0 || responseObj.response.status
          == 9999) && responseObj.response.data) {
        $("#registration_zones option").remove();
        $('#registration_zones').show();
        $('#hidden_registration_zones').hide();

        //var zone = $('#registration_zones');
        $('#registration_zones').addZoneItems('#registration_zones',
            responseObj.response.data, zoneCode);
