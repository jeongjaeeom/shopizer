 ...
 * callback to invoke
 */
function getZones(listDiv, textDiv, countryCode, defaultValue, defaultLongValue,
...
function shippingQuotes(url, useDistanceWindow) {
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      var totalTemplate = Hogan.compile(
          document.getElementById("totalTemplate").innerHTML);
      var quotesTemplate = Hogan.compile(
          document.getElementById("shippingTemplate").innerHTML);
      ...
      if (response.shippingSummary != null) {
        ...
        for (var i = 0; i < summary.shippingOptions.length; i++) {
          if (summary.shippingOptions[i].optionId
              == summary.selectedShippingOption.optionId) {
...
function validateConfirmShipping(shopOrder, useDistanceWindow) {
  ...
  if (shopOrder != null) {
    ...
    if (shopOrder.shippingSummary != null
        && shopOrder.shippingSummary.selectedShippingOption != null) {
...
function displayConfirmShipping(delivery, shippingMethod, useDistanceWindow) {
  ...
  if (useDistanceWindow) {
     ...
     **/
    if (delivery != null && delivery.latitude != null && delivery.longitude
        && delivery.postalCode != null && shippingMethod != null) {
      if (shippingMethod != 'storePickUp') {
        ...
        var map = new google.maps.Map(
            document.getElementById("confirmShippingAddress"), mapOptions);
