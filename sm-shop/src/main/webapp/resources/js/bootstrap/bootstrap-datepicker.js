...
!function ($) {
  ...
  function DatePicker(element, options) {
    this.$el = $(element);
    this.proxy('show').proxy('ahead').proxy('hide').proxy('keyHandler').proxy(
        'selectDate');
  ...
  DatePicker.prototype = {
    ...
    , init: function () {
      ...
      for (var i = 0; i < this.shortDayNames.length; i++) {
        $calendar.append(
            '<div class="dow">' + this.shortDayNames[(i + this.startOfWeek) % 7]
            + '</div>');
      }
      ;
    ...
    , selectMonth: function (date) {
      ...
      if (!this.curMonth || !(this.curMonth.getFullYear()
        ...
        for (var ii = 0; ii <= num_days; ii++) {
          var thisDay = new Date(rangeStart.getFullYear(),
              rangeStart.getMonth(), rangeStart.getDate() + ii, 12, 00);
    ...
    , daysBetween: function (start, end) {
      ...
      var start = Date.UTC(start.getFullYear(), start.getMonth(),
          start.getDate());
    ...
    , findClosest: function (dow, date, direction) {
      ...
      var difference = direction * (Math.abs(
          date.getDay() - dow - (direction * 7)) % 7);
