  ...
  $.flexslider = function (el, options) {
    ...
    methods = {
      init: function () {
        ...
        slider.prop = (vertical) ? "top" : (slider.vars.rtl ? "marginRight"
            : "marginLeft");
        slider.args = {};
        // SLIDESHOW:
        ...
        slider.isFirefox = navigator.userAgent.toLowerCase().indexOf('firefox')
            > -1;
            ...
            for (var i = 0; i < slider.pagingCount; i++) {
              ...
              if ('thumbnails' === slider.vars.controlNav && true
                ...
                if ('' !== captn && undefined !== captn) {
                  var caption = $('<span></span>').addClass(
                      namespace + 'caption').text(captn);
                  item.append(caption);
                }
              }
      ...
      directionNav: {
        setup: function () {
          var directionNavScaffold = $(
              ...
              + '</a></li><li class="' + namespace + 'nav-next"><a class="'
              + namespace + 'next" href="#">' + slider.vars.nextText
              + '</a></li></ul>');
      ...
      pausePlay: {
        ...
        update: function (state) {
          (state === "play") ? slider.pausePlay.removeClass(
              namespace + 'pause').addClass(namespace + 'play').html(
              slider.vars.playText) : slider.pausePlay.removeClass(
              namespace + 'play').addClass(namespace + 'pause').html(
      ...
      touch: function () {
        ...
        if (!msGesture) {
          ...
          onTouchMove = function (e) {
            ...
            dx = (vertical) ? startX - localY : (slider.vars.rtl ? -1 : 1)
                * (startX - localX);
              ...
              (asNav && slider.currentItem === 0 && target
                  ...
                  (target === slider.currentSlide && !asNav) ? false :
                      (slider.vars.animationLoop) ? true :
                          (slider.atEnd && slider.currentSlide === 0 && target
                              === last && slider.direction !== "next") ? false :
    ...
    slider.setProps = function (pos, special, dur) {
      ...
      if (slider.transitions) {
        if (slider.isFirefox) {
          ...
          target = (vertical) ? "translate3d(0," + target + ",0)"
              : "translate3d(" + ((slider.vars.rtl ? -1 : 1) * parseInt(target)
              + 'px') + ",0,0)";
        }
      ...
      if (!fade) {
        ...
        if (type === "init") {
          slider.viewport = $(
              ...
              {"overflow": "hidden", "position": "relative"}).appendTo(
              slider).append(slider.container);
          // INFINITE LOOP:
          slider.cloneCount = 0;
        ...
        if (slider.vars.animationLoop && !carousel) {
          ...
          slider.container.append(methods.uniqueID(
              slider.slides.first().clone().addClass('clone')).attr(
              'aria-hidden', 'true'))
          .prepend(methods.uniqueID(
              slider.slides.last().clone().addClass('clone')).attr(
              'aria-hidden', 'true'));
        }
        ...
        if (vertical && !carousel) {
          slider.container.height(
              (slider.count + slider.cloneCount) * 200 + "%").css("position",
              "absolute").width("100%");
          setTimeout(function () {
        ...
        if (type === "init") {
          if (!touch) {
            ...
            if (slider.vars.fadeFirstSlide == false) {
              slider.slides.css(
                  {"opacity": 0, "display": "block", "zIndex": 1}).eq(
                  slider.currentSlide).css({"zIndex": 2}).css({"opacity": 1});
            } else {
              slider.slides.css(
                  {"opacity": 0, "display": "block", "zIndex": 1}).eq(
                  slider.currentSlide).css({"zIndex": 2}).animate(
            ...
            slider.slides.css({
              "opacity": 0,
              "display": "block",
              "webkitTransition": "opacity " + slider.vars.animationSpeed / 1000
                  + "s ease",
              "zIndex": 1
