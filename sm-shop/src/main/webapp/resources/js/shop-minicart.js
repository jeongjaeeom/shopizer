...
function miniCart(cartCode) {
  ...
  $.ajax({
    type: 'GET',
    url: getContextPath() + '/shop/cart/displayMiniCartByCode?shoppingCartCode='
        + cartCode,
    cache: false,
    error: function (e) {
      console.log('error ' + e);
...
function removeItemFromMinicart(lineItemId) {
  ...
  $.ajax({
    type: 'GET',
    cache: false,
    url: getContextPath() + '/shop/cart/removeMiniShoppingCartItem?lineItemId='
...
function miniCartSummary(cart) {
  if ($('#miniCartSummaryTemplate').length > 0) {
    var miniCartSummaryTemplate = Hogan.compile(
        document.getElementById("miniCartSummaryTemplate").innerHTML);
...
function fullCart(cart) {
  if ($('#miniCartTemplate').length > 0) {
    var miniCartTemplate = Hogan.compile(
        document.getElementById("miniCartTemplate").innerHTML);
