...
if (window.isc && window.isc.module_Core && !window.isc.module_DSBrowser) {
  ...
  isc._moduleStart = isc._DSBrowser_start = (isc.timestamp ? isc.timestamp()
      : new Date().getTime());
  if (isc._moduleEnd && (!isc.Log || (isc.Log && isc.Log.logIsDebugEnabled(
      'loadTime')))) {
    isc._pTM = {
      message: 'DSBrowser load/parse time: ' + (isc._moduleStart
  ...
  isc.B.push(isc.A.initWidget = function isc_DSBrowser_initWidget() {
        ...
        this.dsRegistryListRefreshButton = this.createAutoChild(
            "dsRegistryListRefreshButton");
        ...
        this.leftSection = this.createAutoChild("leftSection", {
          sections: [{
            ...
            title: "DataSources",
            expanded: true,
            controls: [this.dsListAddButton, this.dsListRemoveButton,
              this.dsListRefreshButton],
            items: [this.dsList]
          }]
        });
      ...
      , isc.A.dsRegistryChanged = function isc_DSBrowser_dsRegistryChanged(_1) {
        ...
        isc.DMI.call("isc_builtin", "com.isomorphic.tools.BuiltinRPC",
            "dsFromXML", _1.object, this.getID() + ".dsLoaded(data)")
      }
      ...
      , isc.A.showPane = function isc_DSBrowser_showPane(_1, _2) {
        ...
        isc.addProperties(_3, _1, {
          canClose: true,
          pane: this.createAutoChild(_1.paneClass, {config: _2})
        });
  ...
  isc.A.showSQLBrowserButtonDefaults = {
    ...
    click: function () {
      isc.SQLBrowser.showWindow(
          {width: "95%", height: "95%", isModal: true, autoCenter: true})
    }
  };
  ...
  isc.B.push(isc.A.initWidget = function isc_DSEditor_initWidget() {
        ...
        this.fieldGrid = this.createAutoChild("fieldGrid", {
          fields: [{name: "title", title: "Title"}, {name: "name", title: "Name"},
            {name: "width", title: "Width"}, {name: "height", title: "Height"}, {
              name: "operator",
              title: "Operator",
              valueMap: ["equals", "notEqual", "greaterThan", "lessThan",
                "greaterOrEqual", "lessOrEqual", "contains", "startsWith",
                "endsWith", "iContains", "iStartsWith", "iEndsWith", "notContains",
                "notStartsWith", "notEndsWith", "iNotContains", "iNotStartsWith",
                "iNotEndsWith", "regexp", "iregexp", "isNull", "notNull", "inSet",
                "notInSet", "equalsField", "notEqualField", "and", "not", "or",
                "between", "betweenInclusive"]
            }, {name: "formItemType", title: "Form Item Type"},
            {name: "tableName", title: "Table Name"}, {name: "type", title: "Type"}]
        });
      ...
      , isc.A.execSQL = function isc_DSEditor_execSQL() {
        ...
        if (_1) {
          ...
          _2.performCustomOperation("dsFromSQL",
              {dbName: this.dbList.getSelectedDB(), sql: _1},
      ...
      , isc.A.loadDS = function isc_DSEditor_loadDS(_1) {
        this.currentRegistry = _1;
        isc.DMI.call("isc_builtin", "com.isomorphic.tools.BuiltinRPC",
            "dsConfigFromXML", _1.object, this.getID() + ".dsLoaded(data)")
      }
      ...
      , isc.A.tryIt = function isc_DSEditor_tryIt() {
        ...
        var _2 = isc.DataSource.get("DataSourceStore");
        _2.performCustomOperation("dsFromConfig", {config: _1},
            this.getID() + ".tryItCallback(data)")
      }
  ...
  isc._moduleEnd = isc._DSBrowser_end = (isc.timestamp ? isc.timestamp()
      : new Date().getTime());
  if (isc.Log && isc.Log.logIsInfoEnabled('loadTime')) {
    isc.Log.logInfo(
        'DSBrowser module init time: ' + (isc._moduleEnd - isc._moduleStart)
        + 'ms', 'loadTime');
  }
  ...
  if (window.isc && isc.Log && isc.Log.logWarn) {
    isc.Log.logWarn(
        "Duplicate load of module 'DSBrowser'.");
  }
}
/*
 * Isomorphic SmartClient
