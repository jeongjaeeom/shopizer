...
if (window.isc && window.isc.module_Core && !window.isc.module_SQLBrowser) {
  ...
  isc._moduleStart = isc._SQLBrowser_start = (isc.timestamp ? isc.timestamp()
      : new Date().getTime());
  if (isc._moduleEnd && (!isc.Log || (isc.Log && isc.Log.logIsDebugEnabled(
      'loadTime')))) {
    isc._pTM = {
      message: 'SQLBrowser load/parse time: ' + (isc._moduleStart
  ...
  isc.A.jobsPauseBtnDefaults = {
    ...
    click: function () {
      ...
      var _3 = this;
      QuartzJobs.performCustomOperation("pauseJob",
          {group: _2.group, name: _2.name}, function (_4) {
  ...
  isc.A.jobsResumeBtnDefaults = {
    ...
    click: function () {
      ...
      var _2 = _1.getSelectedRecord();
      var _3 = this;
      QuartzJobs.performCustomOperation("resumeJob",
          {group: _2.group, name: _2.name}, function (_4) {
            _3.creator.triggersGrid.invalidateCache();
            isc.say('Job Resumed')
          })
  ...
  isc.A.jobsTriggerBtnDefaults = {
    ...
    click: function () {
      ...
      var _2 = _1.getSelectedRecord();
      QuartzJobs.performCustomOperation("triggerJob",
          {group: _2.group, name: _2.name}, function (_3) {
  ...
  isc.A.jobsRemoveBtnDefaults = {
    ...
    click: function () {
      var _1 = this;
      isc.ask(
          "Are you sure you wish to delete the selected job?  This will remove all"
          + " triggers associated with this job.", function (_2) {
            if (_2) {
  ...
  isc.A.jobsGridDefaults = {
    ...
    recordDoubleClick: function () {
      isc.say(
          "The Quartz APIs do not allow modification of job metadata without destroying"
    ...
    selectionChanged: function (_1, _2) {
      if (_2) {
        this.creator.triggersGrid.filterData(
            {jobGroup: _1.group, jobName: _1.name})
      } else {
        this.creator.triggersGrid.setData([])
      }
    },
  ...
  isc.A.triggersPauseBtnDefaults = {
    ...
    click: function () {
      ...
      var _2 = _1.getSelectedRecord();
      QuartzTriggers.performCustomOperation("pauseTrigger",
          {group: _2.group, name: _2.name}, function (_3) {
            _1.invalidateCache();
  ...
  isc.A.triggersResumeBtnDefaults = {
    ...
    click: function () {
      ...
      var _2 = _1.getSelectedRecord();
      QuartzTriggers.performCustomOperation("resumeTrigger",
          {group: _2.group, name: _2.name}, function (_3) {
            _1.invalidateCache();
  ...
  isc.A.triggersAddBtnDefaults = {
    ...
    click: function () {
      ...
      this.creator.triggersGrid.startEditingNew(
          {jobGroup: _2.group, jobName: _2.name})
    }
  };
  isc.A.triggersRemoveBtnDefaults = {
    ...
    click: function () {
      var _1 = this;
      isc.ask("Are you sure you wish to remove the selected trigger?",
          function (_2) {
            if (_2) {
              _1.creator.jobsGrid.removeSelectedData(function (_3) {
  ...
  isc.B.push(isc.A.initWidget = function isc_QuartzManager_initWidget() {
        ...
        this.addSection({
          ...
          expanded: true,
          items: [this.jobsGrid],
          controls: [this.jobsPauseBtn, this.jobsResumeBtn, this.jobsTriggerBtn,
            this.jobsRefreshBtn, this.jobsAddBtn, this.jobsRemoveBtn]
        });
        ...
        this.addSection({
          ...
          controls: [this.triggersPauseBtn, this.triggersResumeBtn,
            this.triggersRefreshBtn, this.triggersAddBtn, this.triggersRemoveBtn]
        });
      }
  );
  isc.B._maxIndex = isc.C + 1;
  isc._moduleEnd = isc._SQLBrowser_end = (isc.timestamp ? isc.timestamp()
      : new Date().getTime());
  if (isc.Log && isc.Log.logIsInfoEnabled('loadTime')) {
    isc.Log.logInfo(
        'SQLBrowser module init time: ' + (isc._moduleEnd - isc._moduleStart)
        + 'ms', 'loadTime');
  }
  ...
  if (window.isc && isc.Log && isc.Log.logWarn) {
    isc.Log.logWarn(
        "Duplicate load of module 'SQLBrowser'.");
  }
}
/*
 * Isomorphic SmartClient
