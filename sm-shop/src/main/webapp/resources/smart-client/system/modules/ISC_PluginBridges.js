...
if (window.isc && window.isc.module_Core && !window.isc.module_PluginBridges) {
  ...
  isc._moduleStart = isc._PluginBridges_start = (isc.timestamp ? isc.timestamp()
      : new Date().getTime());
  if (isc._moduleEnd && (!isc.Log || (isc.Log && isc.Log.logIsDebugEnabled(
      'loadTime')))) {
    isc._pTM = {
      message: 'PluginBridges load/parse time: ' + (isc._moduleStart
  ...
  isc.A.instances = [];
  isc.B.push(
      isc.A.handleDragMoveNotify = function isc_c_BrowserPlugin_handleDragMoveNotify() {
      }
  );
  ...
  isc.B.push(isc.A.initComplete = function isc_c_Applet_initComplete(_1) {
        ...
        this.logInfo("ISCEventProxy init complete - jvmVersion: " + _1
            + " - derived version: " + this.jvmVersion)
      }
  ...
  isc.B.push(isc.A.draw = function isc_Applet_draw() {
      ...
      , isc.A.getInnerHTML = function isc_Applet_getInnerHTML() {
        ...
        if (this.useTag == "applet") {
          _1.append("<applet name='", this.getPluginID(),
              "' width='100%' height='100%'", " iscCanvasID='", this.getID(),
              "'");
          if (this.mayScript) {
            _1.append(" mayScript");
          }
          ...
          _1.append("<object classid='", this.classID, "' codebase='",
              this.objectCodeBase, "' width='100%' height='100%'");
          if (this.extraHTML) {
            _1.append(" ", this.extraHTML);
          }
          _1.append(">");
          ...
          if (this.codeBase) {
            _1.append("<param name='codeBase' value='",
                this.codeBase, "'>");
          }
          if (this.archive) {
            _1.append("<param name='archive' value='",
                this.archive, "'>");
          }
          if (this.alt) {
  ...
  isc.B.push(isc.A.flashAvailable = function isc_c_Flashlet_flashAvailable() {
        ...
        if (isc.Browser.isIE) {
          ...
          if (_2 != null) {
            ...
            var _3 = _2.description.substring(16),
                _4 = parseFloat(_3.split(" ")[0]);
            this.$43y = _3;
  ...
  isc.B.push(isc.A.setSrc = function isc_Flashlet_setSrc(_1) {
      ...
      , isc.A.getInnerHTML = function isc_Flashlet_getInnerHTML() {
        ...
        _1.append("<object classid='", this.classID, "' codebase='", _3,
        ...
        _1.append("<embed width='100%' height='100%' name='", this.name,
            "' src=\"", this.src, "\" pluginspage=\"", this.pluginsPage,
            "\" type='", this.type, "'");
        for (var _5 in _4) {
          _1.append(" ", _5, "='", _4[_5], "'");
        }
  ...
  isc.B.push(isc.A.register = function isc_c_SVG_register(_1) {
      ...
      , isc.A.getSVGCanvas = function isc_c_SVG_getSVGCanvas(_1) {
        ...
        var _4 = window[_3];
        if (!_4) {
          this.logError("Can't locate svg instance for id: " + _3
  ...
  isc.B.push(isc.A.draw = function isc_SVG_draw() {
      ...
      , isc.A.handleSVGEvent = function isc_SVG_handleSVGEvent(_1) {
        if (_1.type == "mousemove") {
          if (this.lastMouseMoveX == _1.clientX && this.lastMouseMoveY
              == _1.clientY) {
            return;
          }
      ...
      , isc.A.getInnerHTML = function isc_SVG_getInnerHTML() {
        if (isc.Browser.isIE) {
          return "<embed name='" + this.getPluginID() + "' src=\""
              + isc.Page.getURL(this.src) + "#" + this.getID()
              + "\" width='100%' height='100%'" + (this.installPlugin
                  ? "pluginspage='" + this.pluginsPage + "'" : "")
              + " type='image/svg+xml' " + this.extraHTML + " >"
        }
  ...
  isc.B.push(isc.A.getInnerHTML = function isc_ActiveXControl_getInnerHTML() {
        ...
        _1.append("<object classid='", _2, "' codebase='", this.codeBase,
  ...
  isc.B._maxIndex = isc.C + 3;
  isc._moduleEnd = isc._PluginBridges_end = (isc.timestamp ? isc.timestamp()
      : new Date().getTime());
  if (isc.Log && isc.Log.logIsInfoEnabled('loadTime')) {
    isc.Log.logInfo(
        'PluginBridges module init time: ' + (isc._moduleEnd - isc._moduleStart)
        + 'ms', 'loadTime');
  }
  ...
  if (window.isc && isc.Log && isc.Log.logWarn) {
    isc.Log.logWarn(
        "Duplicate load of module 'PluginBridges'.");
  }
}
/*
 * Isomorphic SmartClient
