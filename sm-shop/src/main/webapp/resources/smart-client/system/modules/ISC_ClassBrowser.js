...
if (window.isc && window.isc.module_Core && !window.isc.module_ClassBrowser) {
  ...
  isc._moduleStart = isc._ClassBrowser_start = (isc.timestamp ? isc.timestamp()
      : new Date().getTime());
  if (isc._moduleEnd && (!isc.Log || (isc.Log && isc.Log.logIsDebugEnabled(
      'loadTime')))) {
    isc._pTM = {
      message: 'ClassBrowser load/parse time: ' + (isc._moduleStart
  ...
  isc.B.push(isc.A.initWidget = function isc_JavaClassPane_initWidget() {
      ...
      , isc.A.loadSource = function isc_JavaClassPane_loadSource() {
        isc.DMI.call("isc_builtin", "com.isomorphic.tools.BuiltinRPC",
            "getJavaSource", this.config.path,
  ...
  isc.B.push(isc.A.initWidget = function isc_ClassBrowser_initWidget() {
      ...
      , isc.A.showClassPane = function isc_ClassBrowser_showClassPane(_1) {
        ...
        this.showPane(
            {ID: _2, title: "Class: " + _1.name, paneClass: "classPane"}, _1)
      }
  ...
  isc._moduleEnd = isc._ClassBrowser_end = (isc.timestamp ? isc.timestamp()
      : new Date().getTime());
  if (isc.Log && isc.Log.logIsInfoEnabled('loadTime')) {
    isc.Log.logInfo(
        'ClassBrowser module init time: ' + (isc._moduleEnd - isc._moduleStart)
        + 'ms', 'loadTime');
  }
  ...
  if (window.isc && isc.Log && isc.Log.logWarn) {
    isc.Log.logWarn(
        "Duplicate load of module 'ClassBrowser'.");
  }
}
/*
 * Isomorphic SmartClient
