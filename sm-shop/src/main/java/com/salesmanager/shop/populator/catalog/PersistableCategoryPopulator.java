...
/*		Validate.notNull(categoryService, "Requires to set CategoryService");
      ...
      if (source.getParent() == null || (StringUtils.isBlank(source.getParent().getCode()))
        ...
        target.setDepth(0);
        target.setLineage(
            new StringBuilder().append("/").append(source.getId()).append("/").toString());
      } else {
      ...
      if (!CollectionUtils.isEmpty(source.getDescriptions())) {
        ...
        if (CollectionUtils.isNotEmpty(target.getDescriptions())) {
          for (com.salesmanager.core.model.catalog.category.CategoryDescription description : target
              .getDescriptions()) {
  ...
  private com.salesmanager.core.model.catalog.category.CategoryDescription buildDescription(
      ...
      com.salesmanager.core.model.catalog.category.CategoryDescription target) throws Exception {
    ...
    if (lang == null) {
      throw new ConversionException("Language is null for code " + source.getLanguage()
