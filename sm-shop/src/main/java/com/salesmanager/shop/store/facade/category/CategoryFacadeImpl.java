...
public class CategoryFacadeImpl implements CategoryFacade {
  ...
  @Override
  public ReadableCategoryList getCategoryHierarchy(MerchantStore store, ListCriteria criteria,
      int depth,
    ...
    try {
      ...
      if (!CollectionUtils.isEmpty(filter) && filter.contains(FEATURED_CATEGORY)) {
        ...
        org.springframework.data.domain.Page<Category> pageable = categoryService
            .getListByDepth(parent, language,
      ...
      readableCategories.stream()
          ...
          .forEach(readableCategory -> {
            ReadableCategory parentCategory = readableCategoryMap
                .get(readableCategory.getParent().getId());
            if (parentCategory != null) {
              parentCategory.getChildren().add(readableCategory);
            }
          });

      List<ReadableCategory> filteredList = readableCategoryMap.values().stream()
          .collect(Collectors.toList());
  ...
  private Category populateCategory(MerchantStore store, PersistableCategory category,
      Category target) {
    try {
      return persistableCatagoryPopulator
          .populate(category, target, store, store.getDefaultLanguage());
  ...
  private void saveCategory(MerchantStore store, Category category, Category parent)
    ...
    List<Category> saveAfter = children.stream()
        .filter(c -> c.getId() == null || c.getId().longValue() == 0).collect(Collectors.toList());
...
/*		if (!CollectionUtils.isEmpty(children)) {
  ...
  public ReadableCategory getById(MerchantStore store, Long id, Language language) {
    try {
      ...
      ReadableCategory readableCategory = categoryReadableCategoryConverter
          .convert(categoryModel, store,
              language);
  ...
  @Override
  public ReadableCategory getCategoryByFriendlyUrl(MerchantStore store, String friendlyUrl,
  ...
  public List<ReadableProductVariant> categoryProductVariants(Long categoryId, MerchantStore store,
    ...
    try {
      List<ProductAttribute> attributes = productAttributeService
          .getProductAttributesByCategoryLineage(store,
      ...
      for (ProductAttribute attr : attributes) {
        ...
        if (attr.getProductOptionValue() != null) {
          ...
          val.setSortOrder(order == null ? attr.getId().intValue()
  ...
  public void setVisible(PersistableCategory category, MerchantStore store) {
    ...
    try {
      ...
      throw new ServiceRuntimeException("Error while getting category [" + category.getId() + "]",
          e);
    }
  }
}
