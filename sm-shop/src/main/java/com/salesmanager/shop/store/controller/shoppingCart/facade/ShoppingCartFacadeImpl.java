...
public class ShoppingCartFacadeImpl
  ...
  //used for api
  private List<com.salesmanager.core.model.shoppingcart.ShoppingCartItem> createCartItems(
      ShoppingCart cartModel,
      List<PersistableShoppingCartItem> shoppingCartItems,
      MerchantStore store) throws Exception {

    List<Long> productIds = shoppingCartItems.stream().map(s -> Long.valueOf(s.getProduct()))
    ...
    List<Product> wrongStoreProducts = products.stream()
        .filter(p -> p.getMerchantStore().getId() != store.getId()).collect(Collectors.toList());
    if (wrongStoreProducts.size() > 0) {
      throw new ResourceNotFoundException(
          "One or more of the items with id's " + wrongStoreProducts.stream()
              .map(s -> Long.valueOf(s.getId())).collect(Collectors.toList())
  ...
  public ShoppingCartData getShoppingCartData(final Customer customer, final MerchantStore store,
    ...
    if (cart.getOrderId() != null && cart.getOrderId().longValue() > 0) {
      if (StringUtils.isNotBlank(shoppingCartId) && !(shoppingCartId
          .equals(cart.getShoppingCartCode()))) {
        cart = shoppingCartService.getByCode(shoppingCartId, store);
      } else {
        return null;
      }
...
/*        List<ShoppingCartItem> unavailables = new ArrayList<ShoppingCartItem>();
  ...
  @Override
  public ShoppingCartData updateCartItems(Optional<String> promoCode,
      final List<ShoppingCartItem> shoppingCartItems, final MerchantStore store,
  ...
  ReadableShoppingCart removeShoppingCartItem(String cartCode, Long productId,
    ...
    for (com.salesmanager.core.model.shoppingcart.ShoppingCartItem shoppingCartItem : cart
        .getLineItems()) {
  ...
  @Override
  public ReadableShoppingCart readableCart(ShoppingCart cart, MerchantStore store,
      Language language) {
    ReadableShoppingCartPopulator readableShoppingCart = new ReadableShoppingCartPopulator();
  ...
  private ReadableShoppingCart modifyCart(ShoppingCart cartModel, PersistableShoppingCartItem item,
    ...
    if (!StringUtils.isBlank(item.getPromoCode()) && StringUtils
        .isBlank(cartModel.getPromoCode())) {
      cartModel.setPromoCode(item.getPromoCode());
      cartModel.setPromoAdded(new Date());
    }
   ...
   * @throws Exception
   */
  private ReadableShoppingCart modifyCartMulti(ShoppingCart cartModel,
      List<PersistableShoppingCartItem> cartItems, MerchantStore store,
      Language language) throws Exception {

    List<com.salesmanager.core.model.shoppingcart.ShoppingCartItem> inCartItemList = createCartItems(
        cartModel, cartItems, store);

    int itemUpdatedCnt = 0;

    Set<com.salesmanager.core.model.shoppingcart.ShoppingCartItem> existingItems = cartModel
        .getLineItems();
    ...
    for (com.salesmanager.core.model.shoppingcart.ShoppingCartItem newItemValue : inCartItemList) {
      ...
      Optional<com.salesmanager.core.model.shoppingcart.ShoppingCartItem> oOldItem =
          existingItems.stream()
              .filter(i -> i.getProductId().intValue() == newItemValue.getProductId().intValue())
              .findFirst();
  ...
  @Override
  public ReadableShoppingCart modifyCart(String cartCode, PersistableShoppingCartItem item,
      MerchantStore store,
      Language language) throws Exception {
  ...
  @Override
  public ReadableShoppingCart modifyCartMulti(String cartCode,
      List<PersistableShoppingCartItem> items, MerchantStore store, Language language)
      throws Exception {
    Validate.notNull(cartCode, "String cart code cannot be null");
  ...
  public void setOrderId(String code, Long orderId, MerchantStore store) throws Exception {
    ...
    if (cart == null) {
      LOG.warn("Shopping cart with code [" + code
          + "] not found, expected to find a cart to set order id [" + orderId + "]");
    } else {
  ...
  @Override
  public ReadableShoppingCart modifyCart(String cartCode, String promo, MerchantStore store,
      Language language)
      throws Exception {
