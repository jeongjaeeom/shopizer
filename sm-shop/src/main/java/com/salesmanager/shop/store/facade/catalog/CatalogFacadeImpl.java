...
public class CatalogFacadeImpl implements CatalogFacade {
  ...
  public void deleteCatalog(Long catalogId, MerchantStore store, Language language) {
    ...
    if (Objects.nonNull(c.getMerchantStore()) && !c.getMerchantStore().getCode()
        .equals(store.getCode())) {
  ...
  public ReadableCatalog getCatalog(String code, MerchantStore store, Language language) {
    ...
    Catalog catalog = catalogService.getByCode(code, store)
        .orElseThrow(
            () -> new ResourceNotFoundException("Catalog with code [" + code + "] not found"));
    return readableCatalogMapper.convert(catalog, store, language);
  }

  @Override
  public void updateCatalog(Long catalogId, PersistableCatalog catalog, MerchantStore store,
      Language language) {
    Validate.notNull(catalogId, "Catalog id cannot be null");
    Validate.isTrue(catalogId > 0, "Catalog id cannot be null");
    Validate.notNull(store, "MerchantStore cannot be null");
    ...
    Catalog c = Optional.ofNullable(catalogService.getById(catalogId))
        .orElseThrow(
            () -> new ResourceNotFoundException("Catalog with id [" + catalogId + "] not found"));
  ...
  @Override
  public ReadableEntityList<ReadableCatalog> getListCatalogs(Optional<String> code,
  ...
  @Override
  public ReadableEntityList<ReadableCatalogCategoryEntry> listCatalogEntry(Optional<String> product,
      Long id, MerchantStore store, Language language, int page, int count) {
    Validate.notNull(store, "MerchantStore cannot be null");
    ...
    Page<CatalogCategoryEntry> entries = catalogEntryService
        .list(catalog, store, language, productCode, page, count);
  ...
  @Override
  public ReadableCatalogCategoryEntry getCatalogEntry(Long id, MerchantStore store,
      Language language) {
  ...
  @Override
  public ReadableCatalogCategoryEntry addCatalogEntry(PersistableCatalogCategoryEntry entry,
    ...
    Catalog catalog = catalogService.getByCode(entry.getCatalog(), store)
        .orElseThrow(
            () -> new ResourceNotFoundException("catalog [" + entry.getCatalog() + "] not found"));

    CatalogCategoryEntry catalogEntryModel = persistableCatalogEntryMapper
        .convert(entry, store, language);
  ...
  @Override
  public void removeCatalogEntry(Long catalogId, Long catalogEntryId, MerchantStore store,
      Language language) {
