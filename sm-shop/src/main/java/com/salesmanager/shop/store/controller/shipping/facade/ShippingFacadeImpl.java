...
public class ShippingFacadeImpl implements ShippingFacade {
  ...
  @Override
  public ExpeditionConfiguration getExpeditionConfiguration(MerchantStore store,
      Language language) {
    ...
    try {
      ...
      if (config != null) {
        expeditionConfiguration.setIternationalShipping(
            config.getShipType() != null && config.getShipType()
                .equals(ShippingType.INTERNATIONAL.name()) ? true : false);
      ...
      throw new ServiceRuntimeException(
          "Error while getting Expedition configuration for store[" + store.getCode() + "]", e);
    }
  ...
  public void saveExpeditionConfiguration(ExpeditionConfiguration expedition, MerchantStore store) {
    ...
    try {
      ...
      config.setShippingType(
          expedition.isIternationalShipping() ? ShippingType.INTERNATIONAL : ShippingType.NATIONAL);
  ...
  private void saveShippingConfiguration(ShippingConfiguration config, MerchantStore store)
    ...
    try {
      ...
      throw new ServiceRuntimeException(
          "Error while saving shipping configuration for store [" + store.getCode() + "]", e);
    }
  }
  ...
  public ReadableAddress getShippingOrigin(MerchantStore store) {
    ...
    if (o == null) {
      throw new ResourceNotFoundException(
          "Shipping origin does not exists for store [" + store.getCode() + "]");
    }
  ...
  public void saveShippingOrigin(PersistableAddress address, MerchantStore store) {
    ...
    try {
      ...
      LOGGER.error("Error while getting shipping origin for country [" + address.getCountry() + "]",
          e);
      throw new ServiceRuntimeException(
          "Error while getting shipping origin for country [" + address.getCountry() + "]", e);
    }
  ...
  public void createPackage(PackageDetails packaging, MerchantStore store) {
    ...
    if (this.packageExists(config, packaging)) {
      throw new OperationNotAllowedException(
          "Package with unique code [" + packaging.getCode() + "] already exist");
    }
  ...
  private boolean packageExists(ShippingConfiguration configuration,
    ...
    List<com.salesmanager.core.model.shipping.Package> packages = configuration.getPackages()
        .stream().filter(p -> p.getCode().equalsIgnoreCase(packageDetails.getCode()))
        .collect(Collectors.toList());
  ...
  private com.salesmanager.core.model.shipping.Package packageDetails(
      ShippingConfiguration configuration, String code) {

    Validate.notNull(configuration, "ShippingConfiguration cannot be null");
    Validate.notNull(code, "PackageDetails code cannot be null");

    List<com.salesmanager.core.model.shipping.Package> packages = configuration.getPackages()
        .stream().filter(p -> p.getCode().equalsIgnoreCase(code)).collect(Collectors.toList());
  ...
  public List<PackageDetails> listPackages(MerchantStore store) {
    ...
    return config.getPackages().stream().map(p -> this.toPackageDetails(p))
        .collect(Collectors.toList());
  ...
  public void updatePackage(String code, PackageDetails packaging, MerchantStore store) {
    ...
    if (p == null) {
      throw new ResourceNotFoundException(
          "Package with unique code [" + packaging.getCode() + "] not found");
    }
    ...
    //need to check if code exists
    List<com.salesmanager.core.model.shipping.Package> packs = config.getPackages().stream()
        .filter(pa -> !pa.getCode().equals(code)).collect(Collectors.toList());
    packs.add(pack);
  ...
  public void deletePackage(String code, MerchantStore store) {
    ...
    List<com.salesmanager.core.model.shipping.Package> packList = config.getPackages().stream()
        .filter(p -> p.getCode().equalsIgnoreCase(code)).collect(Collectors.toList());
            ...
            .map(c -> {
              try {
                ...
                throw new ConversionRuntimeException(
                    "Error converting Country to readable country,e");
              }
            })
