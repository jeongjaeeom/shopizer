...
public class OrderApi {
  ...
  public ReadableOrderList list(@PathVariable final Long id,
    ...
    ReadableOrderList returnList = orderFacade
        .getReadableOrderList(merchantStore, customer, start, count,
            language);
  ...
  public ReadableOrderList list(@RequestParam(value = "page", required = false) Integer page,
    ...
    ReadableOrderList returnList = orderFacade
        .getReadableOrderList(merchantStore, customer, page, count, language);
  ...
  /**
   * This method returns list of all the orders for a store.This is not bound to any specific stores
   * and will get list of all the orders available for this instance
   *
   * @param start
   * @param count
  ...
  public ReadableOrderConfirmation checkout(
    ...
    try {
      ...
      customer = customerFacade
          .populateCustomerModel(customer, order.getCustomer(), merchantStore, language);
      ...
      if (e instanceof CredentialsException) {
        throw new GenericRuntimeException("412",
            "Credentials creation Failed [" + e.getMessage() + "]");
      }
      String message = e.getMessage();
      if (StringUtils.isBlank(message)) {//exception type
        message = "APP-BACKEND";
        if (e
            .getCause() instanceof com.salesmanager.core.modules.integration.IntegrationException) {
