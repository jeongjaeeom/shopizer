...
public class TaxFacadeImpl implements TaxFacade {
  ...
  @Override
  public Entity createTaxClass(PersistableTaxClass taxClass, MerchantStore store,
      Language language) {
    ...
    try {
      ...
      LOGGER.error(
          "Error while saving taxClass [" + taxClass.getCode() + "] for store [" + store.getCode()
              + "]", e);
  ...
  public void deleteTaxClass(Long id, MerchantStore store, Language language) {
    ...
    try {
      ...
      LOGGER.error("Error while getting taxClasse [" + id + "] for store [" + store.getCode() + "]",
          e);
      throw new ServiceRuntimeException(
          "Error while getting taxClasse [" + id + "] for store [" + store.getCode() + "]", e);
    }
  ...
  public ReadableEntityList<ReadableTaxClass> taxClasses(MerchantStore store, Language language) {
    ...
    try {
      ...
      List<ReadableTaxClass> taxClasses = models.stream()
          .map(t -> convertToReadableTaxClass(t, store, language)).collect(Collectors.toList());
      ...
      throw new ServiceRuntimeException(
          "Error while getting taxClasses for store [" + store.getCode() + "]", e);
    }
  }

  private ReadableTaxClass convertToReadableTaxClass(TaxClass t, MerchantStore store,
      Language language) {
    return readableTaxClassMapper.convert(t, store, language);
  }

  @Override
  public void updateTaxClass(Long id, PersistableTaxClass taxClass, MerchantStore store,
      Language language) {
    Validate.notNull(taxClass, "TaxClass cannot be null");
    Validate.notNull(store, "MerchantStore cannot be null");
    Validate.notNull(store.getCode(), "MerchantStore code cannot be null");
    try {
      ...
      if (model == null) {
        ...
        if (!model.getMerchantStore().getCode().equals(store.getCode())) {
          throw new UnauthorizedException(
              "MerchantStore [" + store.getCode() + "] cannot update tax class [" + taxClass
                  .getCode() + "]");
        }
      }
  ...
  public ReadableTaxClass taxClass(String code, MerchantStore store, Language language) {
    ...
    try {
      ...
      LOGGER
          .error("Error while getting taxClass [" + code + "] for store [" + store.getCode() + "]",
              e);
      throw new ServiceRuntimeException(
    ...
    try {
      ...
      LOGGER
          .error("Error while getting taxClass [" + code + "] for store [" + store.getCode() + "]",
              e);
      throw new ServiceRuntimeException(
          "Error while saving taxClass [" + code + "] for store [" + store.getCode() + "]", e);
    }
  }
  ...
  private TaxRate taxRateByCode(String code, MerchantStore store, Language language) {
    ...
    try {
      ...
      if (model == null) {
        throw new ResourceNotFoundException(
            "TaxRate not found [" + code + "] for store [" + store.getCode() + "]");
      }
      if (model != null) {
        if (!model.getMerchantStore().getCode().equals(store.getCode())) {
          throw new UnauthorizedException(
              "MerchantStore [" + store.getCode() + "] cannot get tax rate [" + code + "]");
        }
      }
      return model;
    } catch (ServiceException e) {
      LOGGER.error("Error while getting taxRate [" + code + "] for store [" + store.getCode() + "]",
          e);
      throw new ServiceRuntimeException(
          "Error while getting taxRate [" + code + "] for store [" + store.getCode() + "]", e);
    }
  ...
  private TaxRate taxRateById(Long id, MerchantStore store, Language language) {
    ...
    try {
      ...
      LOGGER
          .error("Error while getting taxRate [" + id + "] for store [" + store.getCode() + "]", e);
      throw new ServiceRuntimeException(
  ...
  public void deleteTaxRate(Long id, MerchantStore store, Language language) {
    ...
    try {
      ...
      LOGGER.error("Error while deleting taxRate [" + id + "] for store [" + store.getCode() + "]",
          e);
      throw new ServiceRuntimeException(
  ...
  public Entity createTaxRate(PersistableTaxRate taxRate, MerchantStore store, Language language) {
    ...
    try {
      ...
      LOGGER.error(
          "Error while saving taxRate [" + taxRate.getCode() + "] for store [" + store.getCode()
              + "]", e);
      throw new ServiceRuntimeException(
          "Error while saving taxRate [" + taxRate.getCode() + "] for store [" + store.getCode()
              + "]", e);
  ...
  @Override
  public void updateTaxRate(Long id, PersistableTaxRate taxRate, MerchantStore store,
      Language language) {
  ...
  public ReadableEntityList<ReadableTaxRate> taxRates(MerchantStore store, Language language) {
    ...
    try {
      ...
      List<ReadableTaxRate> readableRates = rates.stream()
          .map(r -> readableTaxRateMapper.convert(r, store, language)).collect(Collectors.toList());
      ...
      throw new ServiceRuntimeException(
          "Error while getting taxRates for store [" + store.getCode() + "]", e);
    }
