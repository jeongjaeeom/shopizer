...
public class ProductFacadeImpl implements ProductFacade {
  ...
  public ReadableProductList getProductListsByCriterias(MerchantStore store, Language language,
    ...
    Page<Product> modelProductList = productService
        .listByStore(store, language, criterias, criterias.getStartPage(), criterias.getMaxCount());
    ...
    if (relatedItems != null && relatedItems.size() > 0) {
      ...
      for (ProductRelationship relationship : relatedItems) {
        ...
        ReadableProduct proxyProduct = populator
            .populate(relatedProduct, new ReadableProduct(), store,
                language);
  ...
  @Override
  public ReadableProduct getProductBySeUrl(MerchantStore store, String friendlyUrl,
      Language language) throws Exception {

    Product product = productService
        .getBySeUrl(store, friendlyUrl, LocaleUtils.getLocale(language));
  ...
  @Override
  public ReadableProductPrice getProductPrice(Long id, ProductPriceRequest priceRequest,
    ...
    try {
      ...
      List<Long> attrinutesIds = priceRequest.getOptions().stream().map(p -> p.getId())
          .collect(Collectors.toList());

      List<ProductAttribute> attributes = productAttributeService
          .getByAttributeIds(store, model, attrinutesIds);

      for (ProductAttribute attribute : attributes) {
        if (attribute.getProduct().getId().longValue() != id.longValue()) {
          //throw unauthorized
          throw new OperationNotAllowedException(
              "Attribute with id [" + attribute.getId() + "] is not attached to product id [" + id
                  + "]");
        }
      }
