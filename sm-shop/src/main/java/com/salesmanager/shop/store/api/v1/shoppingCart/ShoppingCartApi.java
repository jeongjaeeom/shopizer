...
public class ShoppingCartApi {
  ...
  public ResponseEntity<ReadableShoppingCart> modifyCart(
    ...
    try {
      ReadableShoppingCart cart = shoppingCartFacade
          .modifyCart(code, shoppingCartItem, merchantStore, language);
  ...
  public ResponseEntity<ReadableShoppingCart> modifyCart(
    ...
    try {
      ReadableShoppingCart cart = shoppingCartFacade
          .modifyCart(code, promo, merchantStore, language);
  ...
  @PostMapping(value = "/cart/{code}/multi", consumes = {"application/json"}, produces = {
      "application/json"})
  ...
  public ResponseEntity<ReadableShoppingCart> modifyCart(@PathVariable String code,
    ...
    try {
      ReadableShoppingCart cart = shoppingCartFacade
          .modifyCartMulti(code, Arrays.asList(shoppingCartItems),
  ...
  ReadableShoppingCart getByCustomer(
    ...
    try {
      ...
      throw new ServiceRuntimeException(
          "Exception while getting customer [ " + principal.getName() + "]");
    }

    if (customer == null) {
      throw new ResourceNotFoundException(
          "No Customer found for principal[" + principal.getName() + "]");
    }

    customerFacadev1.authorize(customer, principal);
    ReadableShoppingCart readableCart = shoppingCartFacadev1
        .get(cart, customer.getId(), merchantStore, language);

    if (readableCart == null) {
      throw new ResourceNotFoundException(
          "No cart found for customer [" + principal.getName() + "]");
    }
  ...
  public ResponseEntity<ReadableShoppingCart> deleteCartItem(@PathVariable("code") String cartCode,
      @PathVariable("id") Long itemId, @ApiIgnore MerchantStore merchantStore,
      @ApiIgnore Language language,
      @RequestParam(defaultValue = "false") boolean body) throws Exception {

    ReadableShoppingCart updatedCart = shoppingCartFacade
        .removeShoppingCartItem(cartCode, itemId, merchantStore,
            language, body);
    if (body) {
