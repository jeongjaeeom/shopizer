...
public class PersistableProductTypeMapper implements Mapper<PersistableProductType, ProductType> {
  ...
  @Override
  public ProductType convert(PersistableProductType source, MerchantStore store,
      Language language) {
  ...
  private ProductType type(PersistableProductType type, ProductType destination)
    ...
    if (!CollectionUtils.isEmpty(type.getDescriptions())) {
      ...
      for (ProductTypeDescription d : type.getDescriptions()) {
        com.salesmanager.core.model.catalog.product.type.ProductTypeDescription desc = typeDescription(
            d, destination, d.getLanguage());
      ...
      destination.setDescriptions(
          new HashSet<com.salesmanager.core.model.catalog.product.type.ProductTypeDescription>(
              descriptions));
  ...
  private com.salesmanager.core.model.catalog.product.type.ProductTypeDescription typeDescription(
      ProductTypeDescription description, ProductType typeModel, String lang)
      throws ServiceException {
  ...
  private com.salesmanager.core.model.catalog.product.type.ProductTypeDescription findAppropriateDescription(
      ProductType typeModel, String lang) {
    java.util.Optional<com.salesmanager.core.model.catalog.product.type.ProductTypeDescription> d = typeModel
        .getDescriptions().stream().filter(t -> t.getLanguage().getCode().equals(lang)).findFirst();
    if (d.isPresent()) {
      return d.get();
    } else {
      return null;
