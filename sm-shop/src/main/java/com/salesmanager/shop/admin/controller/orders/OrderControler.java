...
public class OrderControler {
  ...
  public String saveOrder(
    ...
    if (BooleanUtils.toBoolean(coreConfiguration.getProperty("MAIL_SEND_ORDER_UPDATES"))
        && (newOrder.getStatus() != null && !newOrder.getStatus()
        .equals(entityOrder.getOrder().getStatus()))) {
  ...
  private void addStatusUpdatedInfo(com.salesmanager.shop.admin.model.orders.Order entityOrder,
      Customer customer, Locale customerLocale, Map<String, String> templateTokens) {
    ...
    if (StringUtils.isNotEmpty(lastHistory.getComments())) {
      templateTokens.put(EmailConstants.EMAIL_TEXT_STATUS_COMMENTS, messages
          .getMessage("email.order.comments", new String[]{lastHistory.getComments()},
              customerLocale));
    }
    String[] statusMessageText = {String.valueOf(entityOrder.getOrder().getId()),
        DateUtil.formatDate(entityOrder.getOrder().getDatePurchased())};
    String status = messages
        .getMessage("label.order." + entityOrder.getOrder().getStatus().name(), customerLocale,
            entityOrder.getOrder().getStatus().name());
  ...
  private OrderStatusHistory getOrderStatusHistory(
      com.salesmanager.shop.admin.model.orders.Order entityOrder, Locale customerLocale) {
    OrderStatusHistory lastHistory = null;
    List<OrderStatusHistory> lastHistories = orderStatusHistoryService
        .findByOrder(entityOrder.getOrder());
  ...
  private void addStatusChangedHistory(com.salesmanager.shop.admin.model.orders.Order entityOrder,
      Locale locale,
    ...
    orderStatusHistory.setComments(messages.getMessage("email.order.status.changed",
        new String[]{newOrder.getStatus().name(), entityOrder.getOrder().getStatus().name()},
        locale));
    orderStatusHistory.setCustomerNotified(1);
    orderStatusHistory.setStatus(entityOrder.getOrder().getStatus());
    orderStatusHistory.setDateAdded(new Date());
