...
public class CategoryController {
  ...
  private String displayCategory(Long categoryId, Model model, HttpServletRequest request)
    ...
    if (categoryId != null && categoryId != 0) {//edit mode
      ...
      if (!category.isPresent() || category.get().getMerchantStore().getId().intValue() != store
          .getId().intValue()) {
  ...
  public String saveCategory(
    ...
    if (category.getCategory().getId() != null && category.getCategory().getId() > 0) { //edit entry
      ...
      if (!currentCategory.isPresent()
          || currentCategory.get().getMerchantStore().getId().intValue() != store.getId()
          .intValue()) {
        return "catalogue-categories";
      }
    }
  ...
  ResponseEntity<String> pageCategories(HttpServletRequest request) {
    ...
    try {
      ...
      if (!StringUtils.isBlank(categoryName)) {
        ...
        categoryService
            .listByCodes(store, new ArrayList<>(Collections.singletonList(categoryCode)), language);
      } else {
  ...
  ResponseEntity<String> moveCategory(HttpServletRequest request, Locale locale) {
    ...
    try {
      ...
      Optional<Category> child = Optional
          .ofNullable(categoryService.getById(childId, store.getId()));
      Optional<Category> parent = Optional
          .ofNullable(categoryService.getById(parentId, store.getId()));
  ...
  private boolean isValid(MerchantStore store, Optional<Category> child,
      Optional<Category> parent) {
    return !child.isPresent() || !parent.isPresent() || !child.get().getMerchantStore().getId()
        .equals(store.getId()) || !parent.get().getMerchantStore().getId().equals(store.getId());
  }

  private ResponseEntity<String> getResponseAjax(Locale locale, AjaxResponse resp,
  ...
  private ResponseEntity<String> getResponseAjax(AjaxResponse resp, HttpHeaders httpHeaders,
      int codeAlreadyExist) {
