...
public interface PageableProductAvailabilityRepository extends
    PagingAndSortingRepository<ProductAvailability, Long> {
  ...
  @Query(value = "select distinct p from ProductAvailability p "
      ...
      countQuery = "select  count(p) from ProductAvailability p "
          ...
          + "and pprm.id=?2 "
          + "and (?3 is null or pm.code like %?3%)")
  Page<ProductAvailability> listByStore(Long productId, Integer storeId, String child,
      Pageable pageable);
